...
for($i=0; $i<sizeof($inp->lokasi); $i++)
    {
        ...
        LEFT JOIN
        (SELECT thn, bln, SUM(CASE WHEN DATE_ADD(LAST_DAY(tanggal), INTERVAL 5 DAY) < NOW() THEN member ELSE 0 END) AS member FROM tt_sync_income_member WHERE id_lokasi = ".$inp->lokasi[$i]." AND thn = ".$inp->tahun." GROUP BY thn, bln)t2 ON (t1.thn = t2.thn AND t1.bln = t2.bln)
        LEFT JOIN
