...
for($i=0; $i<sizeof($inp->lokasi); $i++)
    {
        ...
        LEFT JOIN
        (SELECT thn, bln, tgl, SUM(CASE WHEN DATE_ADD(LAST_DAY(tanggal), INTERVAL 5 DAY) < NOW() THEN member ELSE 0 END) AS member FROM tt_sync_income_member WHERE id_lokasi = ".$inp->lokasi[$i]." AND thn = ".$inp->tahun." AND bln = ".$inp->bulan." GROUP BY thn, bln, tgl)t2 ON (t1.thn = t2.thn AND t1.bln = t2.bln AND t1.tgl = t2.tgl)
        LEFT JOIN
