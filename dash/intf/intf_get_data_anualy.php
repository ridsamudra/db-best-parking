...
for($i=0; $i<sizeof($inp->lokasi); $i++)
    {
        ...
        LEFT JOIN
        (SELECT thn, SUM(CASE WHEN DATE_ADD(LAST_DAY(tanggal), INTERVAL 5 DAY) < NOW() THEN member ELSE 0 END) AS member FROM tt_sync_income_member WHERE id_lokasi = ".$inp->lokasi[$i]." GROUP BY thn)t2 ON (t1.thn = t2.thn)
        LEFT JOIN
